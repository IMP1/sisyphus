...
enum FlairDirectionType {
	NONE, ## No directional information will be shown.
	AXIS, ## The input's axis will be indicated with arrows.
	DIRECTION, ## The input's direction for an axis will be indicated with an arrow.
}

@export_subgroup("Flairs")
@export var show_directional_flairs: FlairDirectionType
...
func _load_texture_path_impl():
	...
	var flair_textures: Array[Texture2D] = []
	if ControllerIcons.is_node_ready() and _can_be_shown():
		...
		if ControllerIcons.get_path_type(path) == ControllerIcons.PathType.INPUT_ACTION:
			...
			flair_textures.append_array(_get_texture_flairs(event, input_type))
		var tex := ControllerIcons.parse_path(path, input_type)
		if tex:
			textures.append(tex)
	_textures = textures
	_flairs = flair_textures
	_reload_resource()

func _get_texture_flairs(event: InputEvent, input_type: int) -> Array[Texture2D]:
	if event.get(&"axis") == null:
		return []
	var axis: int = event.get(&"axis")
	var flair_textures: Array[Texture2D] = []
	match show_directional_flairs:
		FlairDirectionType.AXIS:
			var tex_path := "x-axis.png" if axis == 0 else "y-axis.png"
			flair_textures.append(ControllerIcons.parse_path("flairs/" + tex_path, input_type))
		FlairDirectionType.DIRECTION:
			var directions: Array[String] = ["left.png", "up.png", "right.png", "down.png"]
			var axis_value := ceili(event.get(&"axis_value"))
			var tex_path := directions[axis + axis_value + 1]
			flair_textures.append(ControllerIcons.parse_path("flairs/" + tex_path, input_type))
	return flair_textures
...
func _draw(to_canvas_item: RID, pos: Vector2, modulate: Color, transpose: bool):
	...
	position = pos # Reset position for flairs
	for i in _flairs.size():
		var tex := _flairs[i]
		tex.draw(to_canvas_item, position, modulate, transpose)
...
func _draw_rect(to_canvas_item: RID, rect: Rect2, tile: bool, modulate: Color, transpose: bool):
	...
	for i in range(_textures.size()):
		...
		position.x += size.x
	position = rect.position # Reset position for flairs
	for i in _flairs.size():
		var tex := _flairs[i]
		var size := tex.get_size() * Vector2(width_ratio, height_ratio)
		tex.draw_rect(to_canvas_item, Rect2(position, size), tile, modulate, transpose)
